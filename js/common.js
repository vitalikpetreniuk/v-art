...
function loadScripts()
{
	...
	$('.chapters-line .chapters-list li a').on('click', function(e){
		e.preventDefault();
		let href = $(this).attr('href');

		$(this).parents('.chapters-list').find('li').removeClass('active');
		$(this).parent().addClass('active');

		$('.chapters-content .chapter').removeClass('active');
		$('.chapters-content .chapter'+href).addClass('active');

		if(href === '#chapter-artwork')
		{
			if($('#vart-artwork .artist-wrapper .artist-chapters .chapters-content .chapter-artwork-main .ca-content .ca-artwork-tablist ul.tablist li.active').length === 0)
			{
				$('#vart-artwork .artist-wrapper .artist-chapters .chapters-content .chapter-artwork-main .ca-content .ca-artwork-tablist ul.tablist li').eq(0).addClass('active');
				$('#vart-artwork .artist-wrapper .artist-chapters .chapters-content .chapter-artwork-main .ca-content .tabcontent #artwork-description').addClass('active');
			}
		}
	})


	let team_popup = $('.os-team .team-popup');
	$('.teammate').on('click', function(){
		let tp_image = $(this).find('.tmi-inner').html();
		let tp_name = $(this).find('.teammate-name').text();
		let tp_position = $(this).find('.teammate-position').text();
		let tp_info = $(this).find('.teammate-info').html();
		let tp_details = $(this).find('.teammate-details').html();

		team_popup.find('.tph-image .tmi-inner').html(tp_image);
		team_popup.find('.tph-image .tph-name').html(tp_name);
		team_popup.find('.tph-image .tph-position').html(tp_position);
		team_popup.find('.tph-info').html(tp_info);
		team_popup.find('.tpc-details').html(tp_details);

		$('body').addClass('team-popup-onscreen');
	})
	$('.tpc-close').on('click', function(){
		$('body').removeClass('team-popup-onscreen');

		team_popup.find('.tph-image .tmi-inner').html('');
		team_popup.find('.tph-image .tph-name').html('');
		team_popup.find('.tph-image .tph-position').html('');
	...
	if($('.person').length)
	{
		$('body').on('click', '.person', function(){
			...
			if($this.hasClass('active'))
			{
				$this.find('.person-content').slideUp(100);
			...
			}else
			{
				...
				$('.person').find('.person-content').slideUp(100);

				$this.addClass('active');
				$this.find('.person-content').slideDown(100);
			}
		})
	}
